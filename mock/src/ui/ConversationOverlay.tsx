...
import { AIRPORT_SCENARIO, TAXI_SCENARIO, type ScenarioContent } from "./scenarios";
...
export function ConversationOverlay() {
  ...
  const s: ScenarioContent = scene === "taxi" ? TAXI_SCENARIO : AIRPORT_SCENARIO;

  const onLeave = () => {
    if (scene === "taxi") {
      // Step out of the taxi back into the airport scene
      setScene("airport");
    }
    setMode("world");
  };
    ...
    <div className="absolute inset-0 z-20 pointer-events-none">
      ...
      <header className="absolute top-0 inset-x-0 z-10 px-6 pt-5 flex items-start justify-between pointer-events-none">
        <div className="glass rounded-2xl px-4 py-2.5 ring-1 ring-white/10 flex items-center gap-3 pointer-events-auto">
          <div className="text-lg">{scene === "taxi" ? "🚕" : "✈️"}</div>
          <div>
            <div className="text-[11px] uppercase tracking-widest text-slate-300/70 font-mono">
              {s.locationLabel}
            </div>
            <div className="text-sm font-semibold text-white">{s.locationSub}</div>
          </div>
        </div>
        <div className="glass rounded-2xl px-4 py-2.5 ring-1 ring-white/10 flex items-center gap-4 pointer-events-auto">
          <div>
            ...
            <div className="text-sm font-semibold text-white">{s.goal}</div>
          </div>
        ...
        <div className="glass rounded-2xl ring-1 ring-white/10 shadow-2xl overflow-hidden">
          <div
            className={
              ...
              (scene === "taxi"
                ? "bg-gradient-to-br from-amber-400/30 via-rose-400/20 to-blue-500/30"
                : "bg-gradient-to-br from-amber-400/30 via-pink-400/20 to-blue-500/30")
            }
          >
          ...
          <div className="p-4">
            <div className="flex items-center justify-between mb-3">
              <div>
                <div className="text-base font-bold text-white leading-tight">{s.npcName}</div>
                <div className="text-xs text-slate-300/80">{s.npcRole}</div>
              </div>
        ...
        <div className="max-w-5xl mx-auto glass rounded-3xl ring-1 ring-white/10 shadow-2xl overflow-hidden">
          <div className="flex items-center justify-between px-5 pt-4 pb-2">
            <div className="flex items-center gap-2">
              <div className="text-xs font-bold text-slate-200">{s.npcName}</div>
          ...
          <div className="px-5 py-3 text-lg leading-relaxed">
                ...
                return (
                  <span key={i} className="relative inline-block group">
                    ...
                    <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 whitespace-nowrap glass-light text-slate-900 text-xs px-3 py-2 rounded-lg ring-1 ring-amber-500/30 shadow-2xl invisible group-hover:visible">
                      <span className="block font-bold">{tok.tip}</span>
                    </span>
                  </span>
                );
              }
              return <Tip key={i} word={tok.text} tip={tok.tip} />;
            })}
            ...
            <div className="mt-3 text-base text-slate-400 italic flex items-center gap-2">
              ...
              <span>
                You: <span className="text-slate-200">{s.userPartial}</span>
          ...
          <div className="border-t border-white/5 px-5 py-4 flex items-center gap-4">
            ...
            <div className="flex-1 hidden md:flex items-center gap-3 bg-amber-500/10 ring-1 ring-amber-400/30 rounded-2xl px-4 py-3">
              ...
              <div>
                ...
                <div className="text-xs text-amber-100">{s.hint.body}</div>
              </div>
            </div>
            ...
            >
              ...
              <span>{scene === "taxi" ? "Step out" : "Leave"}</span>
            </button>
          </div>
          ...
          <div className="border-t border-white/5 px-5 py-2.5 flex items-center gap-3 text-[11px]">
            ...
            {s.objective.steps.map((step, i) => (
              <span key={step.label} className="flex items-center gap-1.5">
                <span className={step.done ? "text-slate-300" : step.current ? "text-slate-300" : "text-slate-500"}>
                  {step.label}
                </span>
                {step.done && <span className="text-emerald-400">✓</span>}
                {step.current && <span className="w-3 h-3 rounded-full bg-amber-400 listen-blink"></span>}
                {i < s.objective.steps.length - 1 && <span className="text-slate-600 ml-1">·</span>}
              </span>
            ))}
            <span className="ml-auto text-slate-400 font-mono">{s.objective.pendingTool}</span>
          </div>
        </div>
      </div>
    </div>
...
function StatePill({ state }: { state: "idle" | "listening" | "thinking" | "speaking" }) {
  const map = {
    speaking: { c: "emerald", label: "Speaking" },
    listening: { c: "rose", label: "Listening" },
    thinking: { c: "violet", label: "Thinking…" },
    idle: { c: "slate", label: "Idle" },
  } as const;
  const { c, label } = map[state];
  return (
    <div className={`flex items-center gap-2 bg-${c}-500/15 ring-1 ring-${c}-400/30 rounded-lg px-3 py-2`}>
      <span className="relative flex h-2.5 w-2.5">
        <span className={`animate-ping absolute inline-flex h-full w-full rounded-full bg-${c}-400 opacity-75`}></span>
        <span className={`relative inline-flex rounded-full h-2.5 w-2.5 bg-${c}-400`}></span>
      </span>
      <span className={`text-xs font-semibold text-${c}-200`}>{label}</span>
      <div className="ml-auto flex items-end gap-0.5 h-3">
        <div className={`w-1 bg-${c}-300 speak-bar`} style={{ height: "60%", animationDelay: "0s" }} />
        <div className={`w-1 bg-${c}-300 speak-bar`} style={{ height: "100%", animationDelay: ".15s" }} />
        <div className={`w-1 bg-${c}-300 speak-bar`} style={{ height: "75%", animationDelay: ".05s" }} />
        <div className={`w-1 bg-${c}-300 speak-bar`} style={{ height: "50%", animationDelay: ".25s" }} />
      </div>
    </div>
  );
}
...
function IconBtn({ children, title }: { children: React.ReactNode; title: string }) {
  return (
    <button title={title} className="hover:bg-white/10 rounded-lg w-7 h-7 grid place-items-center text-xs">
      {children}
    </button>
  );
}
...
function ResponseCard({
  ...
  const map = {
    easy: { c: "emerald", label: "Easy" },
    natural: { c: "amber", label: "Natural" },
    challenge: { c: "rose", label: "Challenge" },
  } as const;
  const { c, label } = map[difficulty];
  return (
    <button
      className={`text-left glass rounded-xl p-3 ring-1 ${
        recommended ? "ring-amber-400/40 hover:ring-amber-400/70" : "ring-white/10 hover:ring-" + c + "-400/40"
      } transition relative`}
    >
      {recommended && (
      ...
      <div className={`text-[10px] uppercase tracking-widest text-${c}-300/90 font-mono mb-1 flex items-center gap-1.5`}>
        <span className={`w-1.5 h-1.5 rounded-full bg-${c}-400`}></span>
        {label}
      </div>
