...
export function StartCTA({ visible, flag, label, subline, disabled, onClick }: Props) {
  return (
    <div className="pointer-events-none fixed inset-0 z-30 grid place-items-center">
      <div
        className={`cta-wrap ${visible ? 'show' : ''} pointer-events-auto flex flex-col items-center gap-2.5`}
      >
        <button
          onClick={onClick}
          disabled={disabled}
        ...
        >
          <span className="w-9 h-9 rounded-full bg-[#0b1228] grid place-items-center ring-1 ring-black/10">
        ...
        <span className="text-center text-[12px] text-slate-400">{subline}</span>
      </div>
    </div>
  );
}
